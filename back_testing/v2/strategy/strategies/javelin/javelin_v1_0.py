'''
FILE: benchmark_v0_0.py
AUTHORS:
    -> Jacob Marshall (marshingjay@gmail.com)
WHAT:
    -> This file is used as a benchmark while testing buy models
'''
from v2.strategy.strategies.strategy import Strategy
from v2.strategy.indicators.param import Param
from v2.strategy.indicators.sma import SMA
import numpy as np


'''
CLASS: Benchmark
WHAT:
    -> test
'''

class Javelin(Strategy):
    '''
    ARGS:
        -> None
    RETURN:
        -> None
    WHAT: 
        -> calculates and adds the indicators to self.indicators to be tested
    '''
    def __init__(self, coin_names, entry_models=[], exit_models=[]):
        super().__init__(entry_models, exit_models)
        # adx = ADX(_params=[Param(0,0,0,'period',14)])
        # adx_roc = RateOfChange(_params=[Param(0,0,0,'period',3)], _value='ADX', _appended_name='ADX')
        sma_for_short = SMA(_params=[Param(0,0,0,'period',300)], _value='close', _appended_name='for_short')
        sma_for_long = SMA(_params=[Param(0,0,0,'period',60)], _value='close', _appended_name='for_long')
        sma_long = SMA(_params=[Param(0,0,0,'period',5000)], _value='close', _appended_name='long')
        
        
        self.algo_indicators.extend([sma_for_short, sma_for_long, sma_long])


        
    
        self.stop_loss = dict()
        
        self.profit_goal = dict()
        self.target = dict()
        self.moving_target = dict()
        self.lost_last = dict()
        self.entered_at = dict()
        self.using_long = dict()
        for x in coin_names:
            self.using_long[x] = True
            self.lost_last[x] = False
            self.stop_loss[x] = 0.0
            self.entered_at[x] = 0.0
            self.profit_goal[x] = 0.0

            self.target[x] = 0.0
            self.moving_target[x] = 0.0
            
        # wanna test some indicators?
        # do that here 

    def process(self, data, coin_name):
        
        if self.moving_target[coin_name] > 0:
            minutes_elapsed = int((data.time - self.entered_at[coin_name]) / (60000))
            
            percentage_eroded = minutes_elapsed/(150)
            self.moving_target[coin_name] = max(0, (1-percentage_eroded) * self.target[coin_name])

        if self.stop_loss[coin_name]:
            self.stop_loss[coin_name] = max(self.stop_loss[coin_name], data.close * 0.995)
        
        return

    def calc_entry(self, data, coin_name):
        
        
        time = data.time
        if data.close < data.SMA_for_short * (0.97) and self.entry_models[1][f'{coin_name}_results'][time]:
            
            self.target[coin_name] = -1 * (1 - (data.close / data.SMA_for_long) - 0.03)
            self.moving_target[coin_name] = self.target[coin_name]
            self.entered_at[coin_name] = time
            return True
            pass
            
       
            
        
        return False

    def calc_exit(self, data, coin_name):
        
        if data.close > data.SMA_for_short * (1 + self.moving_target[coin_name]):
                
                self.stop_loss[coin_name] = max(self.stop_loss[coin_name], data.close * 0.999)
        
       
        if data.close < self.stop_loss[coin_name]:
            self.stop_loss[coin_name] = 0.0
            self.entered_at[coin_name] = 0.0
            self.moving_target[coin_name] = 0.0
            self.target[coin_name] = 0.0
            return True
        
        return False