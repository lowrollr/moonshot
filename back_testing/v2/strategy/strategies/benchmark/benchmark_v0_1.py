...
class Benchmark(Strategy):
    ...
    def __init__(self, coin_names, entry_models=[], exit_models=[]):
        ...
        roc_shorter = RateOfChange(_params=[Param(0,0,0,'period',720)], _value='SMA_for_short', _appended_name='shorter')
    ...
    def calc_entry(self, data, coin_name):
        ...
        if prediction and data.close < data.SMA * (1 - max(0.03, -1*(data.RateOfChange))) * data.RateOfChange_shorter > -0.01:
    ...
    def calc_exit(self, data, coin_name):
        ...
        amnt_above = max(-0.015, min(data.RateOfChange / 2, 0.035))
