...
class Trading:
    ...
    def executePM(self, coin_datasets, start_times, strategy, history_size=20, should_print=True, plot=True):
                    ...
                    if time in coin_times[coin]:
                        ...
                        if not coin_info[coin]['in_position']:
                            ...
                            if strategy.calc_exit(row, coin):
                                ...
                                exited_cash, profit = utils.exitPosition(coin_info[coin], self.fees, time)
                                exits[coin].append((row.time, row.close, profit))
        ...
        for coin in coins:
            ...
            if coin_info[coin]['in_position']:
                ...
                exited_cash, profit = utils.exitPosition(coin_info[coin], self.fees, time)
                exits[coin].append((time, coin_info[coin]['last_close_price'], profit))
                self.computeVolumeFee(exited_cash, time)
                cash += exited_cash
                if profit > 0:
                    win_count += 1
    ...
    def computeVolumeFee(self, volume, time):
        ...
        if self.fee_structure != []:
            ...
            # self.larger_fees
            # self.smaller_fees
            while (self.larger_fees != [] and self.trade_vol_val < self.smaller_fees[-1]["start"]) or \
                (self.smaller_fees != [] and not type(self.smaller_fees[-1]['end']) is str and self.trade_vol_val > self.smaller_fees[-1]["end"]):

                if not type(self.smaller_fees[-1]["end"]) is str and self.trade_vol_val > self.smaller_fees[-1]["end"]:
